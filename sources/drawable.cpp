...
GraphicEngine * Drawable::graphicEngine;
...
Drawable::Drawable() {
	...
	rotZ = 0.0f;
	rotationAngle = 0.0f;
	ogl_Xorigin = ogl_Xcorner = ogl_Ycorner = ogl_Yorigin = 0.0f;
	oglTexture = 0;
	collision = NULL;
	virtualDepth = 100;
	clampTexture = true;
...
Drawable::Drawable(Json::Value aConfig) {
	...
	if (!collisionPath.empty())
		collision = graphicEngine->loadTexture(collisionPath, false);
}
...
void Drawable::processDisplay() {
	...
	if (display) {
		graphicEngine->toDisplay.push_back(this);
	}
}

void Drawable::loadTexture(string path) {
	texture = graphicEngine->loadTexture(path, clampTexture);

	oglTexture = graphicEngine->openGLTextures.at(texture);
	setAnimX(animX);
	setAnimY(animY);
}
...
void Drawable::randomize() {
	...
	if (scaleX == -1 || scaleY == -1) {
		...
		scaleX = minScale
				+ static_cast<float>(rand())
						/ (static_cast<float>(RAND_MAX / valueRange));
	...
	if (rotationAngle == -1) {
		rotationAngle = static_cast<float>(rand())
...
AnimatedDrawable::AnimatedDrawable(const AnimatedDrawable& aDrawable) {
	...
	display = true;
	for (std::map<string, Animation *>::const_iterator anElement =
			aDrawable.animations.begin();
			anElement != aDrawable.animations.end(); ++anElement) {
...
AnimatedDrawable& AnimatedDrawable::operator=(
		const AnimatedDrawable& anAnimatedDrawable) {
	/*name = anAnimatedDrawable.name;
	 width = anAnimatedDrawable.width;
	 height = anAnimatedDrawable.height;*/
	for (std::map<string, Animation *>::const_iterator anElement =
			anAnimatedDrawable.animations.begin();
			anElement != anAnimatedDrawable.animations.end(); ++anElement) {
		Animation * aNewAnim = new Animation((*anElement).second);
		aNewAnim->drawable = this;
		animations.insert(make_pair((*anElement).first, aNewAnim));
	}
